...
\begin{proof}
    ...
    Now for the converse statement. The key idea here is to approximate the indicator function $\indi{1}_{[c,d]}$ with an appropriate continuous function. For a given $\vep > 0$, consider the following continuous approximation for the function $\indi{1}_{[c,d]}$
    \begin{align*}
        f(x) =
        \begin{cases} 
    ...
    \begin{align*}
        \indi{1}_{[c,d]}(x) \leq f(x) \leq \indi{1}_{\left[ c- \frac{\vep}{8}, d+\frac{\vep}{8} \right]}(x) 
    \end{align*}
    Rewriting the above inequality,
    \begin{align*}
        \left| f(x) - \indi{1}_{[c,d]}(x) \right| \leq \indi{1}_{\left[ c-\frac{\vep}{8}, c \right]}(x) + \indi{1}_{\left[ d, d+\frac{\vep}{8} \right]}(x)
    \end{align*}
    This means there exists a large enough $N_1$, such that for all $n > N_1$
    \begin{align*}
        \frac{1}{n} \sum_{k=1}^{n} \left( \indi{1}_{\left[ c-\frac{\vep}{8}, c \right]}(q_k) + \indi{1}_{\left[ d, d+\frac{\vep}{8} \right]}(q_k) \right) \leq \frac{\vep}{3}
    \end{align*}
    For that same $N_1$,
    \begin{align}
        \left| \frac{1}{n}\sum_{k=1}^{n}\indi{1}_{[c,d]}(q_k) - \frac{1}{n}\sum_{k=1}^{n}f(q_k) \right| \leq \frac{\vep}{3} \label{ineq:6}
    \end{align}
    Similarly,
    \begin{align}
        \frac{1}{2\pi} \left| \int_{0}^{2\pi} f(x) dx - \int_{0}^{2\pi}\indi{1}_{[c,d]}(x)dx \right| \leq \frac{\vep}{3} \label{ineq:7}
    \end{align}
